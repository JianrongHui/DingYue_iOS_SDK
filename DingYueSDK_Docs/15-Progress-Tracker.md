...
| 里程碑 | 描述 | 进度 | Owner | 状态 |
|--------|------|------|-------|------|
| M1 | 后台基础能力（配置/事件/包管理） | 100% | Backend | ✅ 已完成 |
...
- [x] **Cloudflare Workers + Hono 迁移** _(2026-01-05)_
- [x] **D1 SQLite 迁移脚本** _(2026-01-05)_
- [x] **R2 包管理 + fflate 校验** _(2026-01-05)_
- [x] **Admin 路由（Apps/Placements/Variants/Experiments/Rulesets/Events）** _(2026-01-05)_
...
## Cloudflare 迁移阶段记录（2026-01-05）

| 阶段 | 内容 | 状态 |
|------|------|------|
| Stage 1 | 基础设施准备（wrangler.toml、依赖、env 类型） | ✅ |
| Stage 2 | D1 适配器与数据库访问重构 | ✅ |
| Stage 3 | SQLite 迁移脚本转换 | ✅ |
| Stage 4 | Express → Hono + Worker 入口 | ✅ |
| Stage 5 | R2 包管理 + fflate 校验 | ✅ |
| Stage 6 | Web Admin 部署配置 | ✅ |
| Stage 7 | 本地验证（wrangler dev + 端点测试 + D1 迁移） | ✅ |
